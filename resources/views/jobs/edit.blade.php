<x-layout>
    ...
    <form method="POST" action="/jobs/{{$job->id}}">
                ...
                <div class="mt-10 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                    <div class="sm:col-span-4">
                        ...
                        <div class="mt-2">
                            <div class="flex items-center rounded-md bg-white/5 pl-3 outline-1 -outline-offset-1 outline-black/10 focus-within:outline-2 focus-within:-outline-offset-2 focus-within:outline-indigo-500">
                                ...
                                <input id="title" value={{$job->title}} type="text" name="title" placeholder="AI Engineer" class="block min-w-0 grow bg-transparent py-1.5 pr-3 pl-1 text-base text-black placeholder:text-gray-500 focus:outline-none sm:text-sm/6 border-2 border-b-gray-700" required />
                            </div>
                            @error('title')
                    ...
                    <div class="col-span-full ">
                        ...
                        <div class="flex items-center rounded-md bg-white/5  outline-1 -outline-offset-1 outline-black/10 focus-within:outline-2 focus-within:-outline-offset-2 focus-within:outline-indigo-500">
                            <input id="salary" type="text" value={{$job->salary}} name="salary" placeholder="Salary" class="block min-w-0 bg-transparent py-1.5 pr-3 pl-1 text-base text-black placeholder:text-gray-500 focus:outline-none sm:text-sm/6 border-2 border-b-gray-700 ml-3" required />
                        </div>
                        @error('salary')
                        <p class="p-3 text-red-500">{{$message}}</p>
                        @enderror
        ...
        <div class="mt-6 flex items-center justify-end gap-x-6">
            <button href="/jobs/{{$job->id}}" class="text-sm/6 font-semibold text-black-50">Cancel</button>
